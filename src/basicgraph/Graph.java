...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		int[] degrees = new int[this.getNumVertices()]; // length of the list is the num of vertices
		for(int v = 0; v < this.getNumVertices(); v++){
			degrees[v] = getNeighbors(v).size() + getInNeighbors(v).size(); // degree of vertice v
		}
		Arrays.sort(degrees);
		List<Integer> degSeq = new ArrayList<Integer>(); 
		for(int deg : degrees){ // insert degrees into degSeq in reversed order
			degSeq.add(0, deg);
		}
