...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		// using square matrix multiplication to find the distance 2 neighbors
		int[][] adjMatrix_two_hop = new int[this.getNumVertices()][this.getNumVertices()];
		for(int row = 0; row < this.getNumVertices(); row++){
			for(int col = 0; col < this.getNumVertices(); col++){
				for(int index = 0; index < this.getNumVertices(); index++){
					adjMatrix_two_hop[row][col] += adjMatrix[row][index] * adjMatrix[index][col];
				}
			}
		}
		List<Integer> neighbors_two_hop = new ArrayList<Integer>();
		for(int i = 0; i < this.getNumVertices(); i++){
			for(int j = 0; j < adjMatrix_two_hop[v][i]; j++){
				neighbors_two_hop.add(i);
			}
		}
		
		return neighbors_two_hop;
	}
